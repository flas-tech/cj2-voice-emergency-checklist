...
## 5. How the firmware is organised

```
firmware/
  main/
    main.c           AFE + WakeNet + MultiNet pipeline, PTT, checklist state machine
    checklists.c/.h  all 12 CJ2 checklists (triggers, items, advance words)
    audio_player.c/.h WAV playback over I2S
    board_pins.h     GPIO map (edit for your board)
    idf_component.yml pulls espressif/esp-sr
    CMakeLists.txt   builds + embeds audio into SPIFFS
  scripts/make_audio.py   renders the spoken clips
  partitions.csv     factory app + model + storage(audio) partitions
...
| **"clip not found"** in log | You flashed before generating clips. Run `make_audio.py`, then `idf.py build flash`. |
